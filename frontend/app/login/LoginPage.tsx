...
import { Activity, Target, ShieldCheck, Mail, Lock } from "lucide-react";
          ...
          <div className="lg:col-span-4 hidden lg:flex flex-col justify-center">
            ...
            >
              ...
              <h2 className="text-[3rem] xl:text-[4.5rem] font-serif font-bold leading-[0.95] text-[#1F363D] tracking-tighter">
                Eat <span className="text-[#40798C]">Better,</span><br />
                Live <span className="text-[#40798C]">Longer.</span>
              </h2>
              ...
              >
                ...
                <div className="flex items-center justify-between px-0.5 mt-1">
                  <label className="flex items-center gap-1.5 cursor-pointer group select-none">
                    <input
                      type="checkbox"
                      className="w-3.5 h-3.5 border-2 border-[#40798C]/30 rounded-[4px] appearance-none checked:bg-[#40798C] checked:border-[#40798C] transition-all cursor-pointer"
                    />
                  ...
                  <button className="text-[10px] font-black text-[#40798C] hover:text-[#1F363D] transition-colors cursor-pointer uppercase tracking-[0.05em]">
                    Forgot Password?
                  </button>
                </div>
                ...
                <button
                  onClick={handleLogin}
                  disabled={loading}
                  className="w-full bg-[#40798C] text-[#F2F7F2] h-14 flex items-center justify-center gap-3 rounded-2xl font-bold text-sm hover:bg-[#1F363D] hover:text-white shadow-lg shadow-[#40798C]/10 transition-all active:scale-[0.98] disabled:opacity-60 disabled:cursor-not-allowed cursor-pointer tracking-widest uppercase mt-4"
                >
                  {loading && (
...
function FeatureStrip({ icon, label }: { icon: React.ReactNode; label: string }) {
  return (
    ...
    >
      ...
      <span className="text-sm font-bold text-[#1F363D]/90 tracking-wide leading-snug">{label}</span>
    </motion.div>
  );
}
